...
/*
   void change_speed(float speed, float angle, float *wheel_control, float
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber controller_sub =
      n.subscribe("controller_info", 10, joy_callback);
  ros::Subscriber gyro_sub = n.subscribe("gyro_info", 10, gyro_callback);
  ros::ServiceClient motor_speed =
      n.serviceClient<motor_serial::motor_serial>("motor_info");
  ...
  while (ros::ok()) {
    std_msgs::Int16 msg;
    change_speed(robot_speed, robot_angle, wheel_control, robot_right,
                 robot_left);
    for (int i = 0; i < 3; ++i) {
      if (speed_half == true) {
        wheel_control[i] /= 2;
