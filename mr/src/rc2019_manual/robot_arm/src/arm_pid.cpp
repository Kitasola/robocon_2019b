...
void armCallback(const std_msgs::Float64MultiArray &msg){
	goal_angle[0] = msg.data[0] * (180 / M_PI);
	goal_angle[1] = msg.data[1] * (180 / M_PI);
}

void calibrationCallback(const std_msgs::Bool &msg){
	flag_calibration = msg.data;
}

int main(int argc, char **argv){
	ros::init(argc, argv, "arm_pid");
	ros::NodeHandle n;
	ros::Subscriber calibration_sub = n.subscribe("arm_calibration", 10, calibrationCallback);
	ros::Subscriber angle_sub = n.subscribe("arm_angle", 10, armCallback);
